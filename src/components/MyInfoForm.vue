  ...
  <VForm v-slot="{ errors }" @submit="onSubmit">
    <fieldset :disabled="isLoadingSubmit" class="row g-3">
      ...
      <template v-if="!user?.id">
        ...
        <div class="col-md-6">
          <label :for="formSchema.password.name" class="form-label"
            >{{ formSchema.password.label }}<span class="text-danger" v-if="formSchema.password.isRequired">*</span></label
          >
          <Field
          ...
          <span v-if="!errors[formSchema.password.label]" class="invalid-feedback form-help">{{ formSchema.password.help }}</span>
        </div>
      </template>
      <div class="col-md-6">
        <label :for="formSchema.userName.name" class="form-label"
          >{{ formSchema.userName.label }}<span class="text-danger" v-if="formSchema.userName.isRequired">*</span></label
        >
        <Field
      ...
      <div class="col-12 border-top pt-3">
        <div class="alert alert-warning" role="alert">警告：請確實勾選您的證照，出團時，須提供證件審查；如有不實，則受退團處分，且不予退費。</div>
        <label class="form-label d-block"
          >{{ formSchema.certificateLevel.label }}<span class="text-danger" v-if="formSchema.certificateLevel.isRequired">*</span></label
        >
        <div class="form-check form-check-inline" v-for="option in certificateLevels" :key="option.id">
          ...
          /><label class="form-check-label" :for="`${formSchema.certificateLevel.name}-${option.id}`">{{ option.name }}</label>
        </div>
