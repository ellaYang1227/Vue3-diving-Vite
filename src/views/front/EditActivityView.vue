        ...
        <fieldset :disabled="isLoadingBtn" class="row g-3">
          <div class="col-12">
            <label class="form-label d-block"
              >{{ formSchema.activity.imgs.label }}<span class="text-danger" v-if="formSchema.activity.imgs.isRequired">*</span></label
            >
            <div class="row g-3">
              <div class="col-auto" v-for="(imgItem, key) in form.imgs" :key="key">
                <UploadImg
                  :errors="errors"
                  :showErrMsg="false"
                  :isRequired="imgItem.isMain && !imgItem.img"
                  :inputName="key"
                  v-model:img="imgItem.img"
                >
                  <template #imgFunctions>
                    <div class="img-functions cursor-default" :class="{ 'opacity-90': imgItem.isMain }" @click.prevent.self>
                      <template v-if="!imgItem.isMain">
                        <button
                          type="button"
                          class="btn btn-link btn-sm fs-8 text-lightPrimary text-decoration-none flex-shrink-0 px-0"
                          :disabled="isLoadingBtn"
                          @click.self="setMainImg(key)"
                        >
                          設為封面
                        </button>
                        <button
              ...
              <div class="col-auto" v-if="3 > Object.keys(form.imgs).length">
                <button type="button" class="btn opacity-50 file-img" :disabled="isLoadingBtn" @click="changeImg()">
                  <font-awesome-icon icon="fa-solid fa-plus" size="2x" class="icon-color" />
                </button>
              </div>
            </div>
          ...
          <div class="col-md-12">
            <label :for="formSchema.activity.title.name" class="form-label"
              >{{ formSchema.activity.title.label }}<span class="text-danger" v-if="formSchema.activity.title.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-12">
            <label :for="formSchema.activity.features.name" class="form-label"
              >{{ formSchema.activity.features.label }}<span class="text-danger" v-if="formSchema.activity.features.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-12">
            <label :for="formSchema.activity.content.name" class="form-label"
              >{{ formSchema.activity.content.label }}<span class="text-danger" v-if="formSchema.activity.content.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-4">
            <label :for="formSchema.activity.startDate.name" class="form-label"
              >{{ formSchema.activity.startDate.label }}<span class="text-danger" v-if="formSchema.activity.startDate.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-4">
            <label :for="formSchema.activity.endDate.name" class="form-label"
              >{{ formSchema.activity.endDate.label }}<span class="text-danger" v-if="formSchema.activity.endDate.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-4">
            <label :for="formSchema.activity.locationId.name" class="form-label"
              >{{ formSchema.activity.locationId.label }}<span class="text-danger" v-if="formSchema.activity.locationId.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-4">
            ...
            <Field
              ...
              :type="formSchema.activity.maxOrderTotal.type"
              :min="formSchema.activity.maxOrderTotal.min"
          ...
          <div class="col-md-4">
            <label :for="formSchema.activity.cost.name" class="form-label"
              >{{ formSchema.activity.cost.label }}<span class="text-danger" v-if="formSchema.activity.cost.isRequired">*</span></label
            >
            <div class="input-group">
              ...
              <Field
                ...
                :min="formSchema.activity.cost.min"
          ...
          <div class="col-md-4">
            <label :for="formSchema.cylinderTotal.name" class="form-label d-block"
              >{{ formSchema.cylinderTotal.label }}<span class="text-danger" v-if="formSchema.cylinderTotal.isRequired">*</span></label
            >
            <Field
          ...
          <div class="col-md-4 align-self-center">
            <label class="form-label d-block"
              >{{ formSchema.isNitrox.label }}<span class="text-danger" v-if="formSchema.isNitrox.isRequired">*</span></label
            >
            <div class="form-check form-check-inline" v-for="option in formSchema.isNitrox.options" :key="option">
              ...
              /><label class="form-check-label" :for="`${formSchema.isNitrox.name}-${option}`">{{ option ? "是" : "否" }}</label>
            </div>
          ...
          <div class="col-md-12">
            <label :for="formSchema.activity.tags.name" class="form-label d-block"
              >{{ formSchema.activity.tags.label }}<span class="text-danger" v-if="formSchema.activity.tags.isRequired">*</span></label
            >
            <div class="row g-1">
