...
export default {
  ...
  methods: {
    ...
    onSubmit(search) {
      ...
      this.filterMyOrders = this.myOrders.filter(
        order =>
  ...
  <div class="py-3">
    ...
    <template v-if="filterMyOrders.length">
      <div
        ...
        v-for="(filterOrder, index) in filterMyOrders"
        :key="filterOrder.id"
      >
        <div class="row g-3 align-items-sm-center">
          ...
          <div class="col-6 col-xl col-xxl-2 d-flex flex-column justify-content-sm-center align-items-start">
            ...
            <span :class="{ 'text-danger': filterOrder.activity.activityStatus === 0, 'opacity-50': filterOrder.activity.activityStatus === 3 }">{{
              activityStatusTextFormat(filterOrder.activity.activityStatus)
            }}</span>
          </div>
          <div class="col-xl-2">
            <div class="row gx-2 align-items-center">
              <div class="col-auto">
                <RouterLink :to="`/activity/${filterOrder.activity.id}`" class="btn btn-link btn-sm border-0 p-0">詳情</RouterLink>
              </div>
              <div class="col-auto">
                <RouterLink :to="`/activity/${filterOrder.activity.id}#messaget`" class="btn btn-link btn-sm border-0 p-0">留言</RouterLink>
              </div>
              <div class="col-auto">
                <button
                  type="button"
                  class="btn btn-link btn-sm border-0 p-0"
                  @click="openCommentModal(filterOrder.activity)"
                  :disabled="filterOrder.activity.activityStatus === 0 || filterOrder.activity.activityStatus === 1"
                >
                  評論<template v-if="filterOrder.activity.comment">(1)</template>
                </button>
              </div>
              <div class="col-auto col-xl-12">
                <button
                  type="button"
                  class="btn btn-link btn-sm border-0 p-0"
                  :disabled="filterOrder.activity.orderStatus === 3 || filterOrder.activity.orderStatus === 0"
                  @click="openDeleteOrderModal(filterOrder)"
                >
                  刪除報名
                </button>
              </div>
    ...
    <div class="border-top px-3 pt-2 opacity-80">
      <ul class="row gx-2 gx-md-3 align-items-center list-unstyled mb-0">
        <li class="col-md">
          總計：<strong class="me-1 font-barlow">{{ filterMyOrders.length }}</strong
          >筆
        </li>
        <li class="col-auto ms-md-auto" :class="{ 'text-danger': getActivityStatusTotal(0) }">
          系統中止：<strong class="me-1 font-barlow">{{ getActivityStatusTotal(0) }}</strong
          >筆
        </li>
        <li class="col-auto">
          未開始：<strong class="me-1 font-barlow">{{ getActivityStatusTotal(1) }}</strong
          >筆
        </li>
        <li class="col-auto">
          進行中：<strong class="me-1 font-barlow">{{ getActivityStatusTotal(2) }}</strong
          >筆
        </li>
        <li class="col-auto">
          已結束：<strong class="me-1 font-barlow">{{ getActivityStatusTotal(3) }}</strong
          >筆
        </li>
      </ul>
    </div>
