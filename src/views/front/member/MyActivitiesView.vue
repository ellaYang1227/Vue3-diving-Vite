...
export default {
  data() {
    return {
      dateFormat,
      activityStatusTextFormat,
      orderStatusTextFormat,
      getMainImg,
      filterMyActivities: [],
      currentActivity: {},
  ...
  methods: {
    ...
    onSubmit(search) {
      ...
      this.filterMyActivities = this.myActivities.filter(
        activity =>
  ...
  <div class="py-3">
    <div class="row g-2 pb-3">
      ...
      <div class="col-md-auto">
        <RouterLink class="btn btn-primary btn-custom-rectangle" to="/addActivity" role="button">新增揪團</RouterLink>
      </div>
    </div>
    <template v-if="filterMyActivities.length">
      <div
        class="p-3"
        :class="{ 'bg-lightPrimary bg-opacity-10': (index + 1) % 2 }"
        v-for="(filterActivity, index) in filterMyActivities"
        :key="filterActivity.id"
      >
        <div class="row g-3 align-items-sm-center">
          ...
          <div class="col-6 col-xl d-flex flex-column justify-content-sm-center align-items-start">
            ...
            <span class="font-barlow"
              ><strong>{{ filterActivity.orders.length }}</strong
              >/{{ filterActivity.maxOrderTotal }}</span
            >
          </div>
          <div class="col-6 col-xl d-flex flex-column justify-content-sm-center align-items-start">
            <small class="order-md-last opacity-80">揪團狀態</small>
            <span :class="{ 'text-danger': filterActivity.orderStatus === 0, 'opacity-50': filterActivity.orderStatus === 2 }">{{
              orderStatusTextFormat(filterActivity.orderStatus)
            }}</span>
          </div>
          <div class="col-6 col-xl d-flex flex-column justify-content-sm-center align-items-start">
            <small class="order-md-last opacity-80">活動狀態</small>
            <span :class="{ 'text-danger': filterActivity.activityStatus === 0, 'opacity-50': filterActivity.activityStatus === 3 }">{{
              activityStatusTextFormat(filterActivity.activityStatus)
            }}</span>
          </div>
          <div class="col-6 col-xl d-flex flex-column justify-content-sm-center align-items-start">
          ...
          <div class="col-xxl-2">
            <div class="row gx-2 align-items-center" v-if="filterActivity.activityStatus !== 0">
              <div class="col-auto">
                <RouterLink
                  :to="`/editActivity/${filterActivity.id}`"
                  class="btn btn-link btn-sm border-0 p-0"
                  :class="{ disabled: filterActivity.orders.length }"
                  >編輯</RouterLink
                >
              </div>
              <div class="col-auto">
                <RouterLink :to="`/activity/${filterActivity.id}`" class="btn btn-link btn-sm border-0 p-0">查看</RouterLink>
              </div>
              <div class="col-auto">
                <button
                  type="button"
                  class="btn btn-link btn-sm border-0 p-0"
                  :disabled="!filterActivity.orders.length"
                  @click="openOrderModal(filterActivity.id)"
                >
                  報名總覽
                </button>
              </div>
              <div class="col-auto">
                <RouterLink :to="`/activity/${filterActivity.id}#messaget`" class="btn btn-link btn-sm border-0 p-0">留言</RouterLink>
              </div>
              <div class="col-auto">
                <button
                  type="button"
                  class="btn btn-link btn-sm border-0 p-0"
                  :disabled="filterActivity.orderStatus === 3"
                  @click="openDelActivityModal(filterActivity)"
                >
                  刪除
                </button>
              </div>
            </div>
            <button type="button" class="btn btn-link btn-sm border-0 p-0" v-else @click="openViolationModal(filterActivity.violations[0]?.id)">
              違規原因
            </button>
          </div>
        </div>
    ...
    <div class="border-top px-3 pt-2 opacity-80">
      <ul class="row gx-2 gx-md-3 align-items-center list-unstyled mb-0">
        <li class="col-md">
          總計：<strong class="me-1 font-barlow">{{ filterMyActivities.length }}</strong
          >筆
        </li>
        <li class="col-auto ms-md-auto" :class="{ 'text-danger': getOrderStatusTotal(0) }">
          系統中止：<strong class="me-1 font-barlow">{{ getOrderStatusTotal(0) }}</strong
          >筆
        </li>
        <li class="col-auto">
          已額滿：<strong class="me-1 font-barlow">{{ fullActivitiesTotal }}</strong
          >筆
        </li>
        <li class="col-auto">
          進行中：<strong class="me-1 font-barlow">{{ getOrderStatusTotal(2) }}</strong
          >筆
        </li>
        <li class="col-auto">
          已截止：<strong class="me-1 font-barlow">{{ getOrderStatusTotal(3) }}</strong
          >筆
        </li>
      </ul>
    </div>
