    ...
    submitOrder() {
      if (!this.user) {
        ...
        if (id == userId) {
          ...
          if (this.activity) {
            if (
              this.activity.certificateLevelId > certificateLevelId ||
              ...
              this.updateOrder(this.activityId, findOrder?.id).then(resArr => {
    ...
    <div class="container py-2">
      <div class="row gy-2">
        ...
        <div class="col-sm-6 d-flex align-items-center justify-content-sm-end">
          ...
          <button
            type="button"
            class="ms-auto ms-sm-0 btn btn-custom-rectangle"
            :disabled="isLoadingBtn || activityBtnDisabled"
            :class="activityBtnDisabled ? 'btn-lightPrimary opacity-40' : 'btn-danger'"
            @click="submitOrder"
          >
            <span class="spinner-border spinner-border-sm" role="status" v-if="isLoadingBtn"></span> {{ activityBtnText }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="row gy-4 align-items-center">
      ...
      <div class="col-md-6 py-md-3">
        ...
        <ul class="list-unstyled opacity-80 mb-1">
          <li class="font-barlow">
            <font-awesome-icon icon="fa-solid fa-calendar-days" fixed-width class="me-1" />{{ dateFormat(activity.startDate) }} ~
            {{ dateFormat(activity.endDate) }}
          </li>
          <li class="font-barlow">
            <font-awesome-icon icon="fa-solid fa-user" fixed-width class="me-1" /><strong>{{ validOrderTotal }}</strong
            >{{ ` / ${activity.maxOrderTotal}` }}
          </li>
          <li class="mt-3">
            主揪人<strong class="ms-2">{{ activity.user?.name }}</strong>
          </li>
          <li>
            證照等級<strong class="ms-2">{{ activity.certificateLevel?.name }}</strong>
          </li>
          <li>
            需有高氧證照<strong class="ms-2">
              <template v-if="activity.isNitrox">是</template>
              <template v-else>否</template>
            </strong>
          </li>
          <li>
            潛水支數<strong class="ms-2">{{ activity.cylinderTotalId ? activity.cylinderTotal.name : "不限" }}</strong>
          </li>
          <li class="font-barlow">
            報名截止<strong class="ms-2">{{ dateFormat(activity.orderExpiryDate) }}</strong>
          </li>
        </ul>
        <RouterLink
          class="text-decoration-none badge bg-lightPrimary bg-opacity-20 me-1"
          v-for="tag in activity.tags"
          :key="tag"
          :to="{ path: '/activities', query: { tag: tag } }"
          >{{ tag }}</RouterLink
        >
        <div class="mt-3 d-flex align-items-center">
          ...
          <button
            ...
            :class="
              activityBtnDisabled
                ? activity.orderStatus === 4 && activity.activityStatus === 3
                  ? 'btn-danger'
                  : 'btn-lightPrimary opacity-40'
                : 'btn-danger'
            "
            @click="submitOrder"
          >
