...
export default defineStore("OrderStore", {
  ...
  actions: {
    ...
    getSearchOrdersForActivityId(activityId) {
      const paramsArr = ["_expand=user", "_sort=updateDate", "_order=desc", "isDelete=0", `activityId=${activityId}`];

      return bacsRequest
        .get(`440/orders?${paramsArr.join("&")}`)
        .then(res => Promise.resolve(res))
        .catch(err => Promise.reject(false));
    },
    /**
    ...
    updateOrder(activityId, orderId) {
      ...
      if (!orderId) {
        ...
        (apiMethod = "post"), (apiUrl = `660/users/${getStorageUser()?.id}/orders`);
      }
      ...
      return bacsRequest[apiMethod](apiUrl, body)
        .then(res => {
          ...
          return Promise.all([getActivity(activityId), getMyOrders(3)]);
        })
        .catch(({ status }) => {
          if (status !== 401) {
