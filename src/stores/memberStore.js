...
export default defineStore("MemberStore", {
  ...
  actions: {
     ...
     */
    getMyinfo() {
      const paramsArr = ["_expand=certificateLevel", "_expand=cylinderTotal", "_embed=violations", "_embed=comments"];
     ...
     */
    getMyOrders(count) {
      const paramsArr = ["_sort=updateDate", "_order=desc", "isDelete=0", "_expand=activity"];

      return bacsRequest
        .get(`400/users/${getStorageUser()?.id}/orders?${paramsArr.join("&")}`)
        .then(orders => {
          // 過濾活動已刪除
    ...
    updateActivity(body) {
      ...
      return bacsRequest[apiMethod](apiUrl, body)
        .then(res => {
     ...
     */
    getMyActivities() {
      const paramsArr = ["_sort=updateDate", "_order=desc", "_expand=location", "_embed=violations", "_embed=orders", "isDelete=0"];

      return bacsRequest
        .get(`600/users/${getStorageUser()?.id}/activities?${paramsArr.join("&")}`)
        .then(res => {
