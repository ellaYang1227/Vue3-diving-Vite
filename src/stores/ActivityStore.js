...
export default defineStore("ActivityStore", {
  ...
  actions: {
    ...
    getNewActivities() {
      ...
      return bacsRequest
        .get(`${this.activitiesApiUrl}`, { params })
    ...
    getSearchActivitiesForActivityId(activityIds) {
      const paramsArr = ["_expand=user", "_expand=location", "_embed=violations", "_embed=orders", "_embed=comments"];

      if (activityIds.length) {
        paramsArr.push(activityIds.join("&"));
      }

      return bacsRequest
        .get(`440/activities?${paramsArr.join("&")}`)
        .then(res => Promise.resolve(res))
        .catch(err => Promise.reject(false));
    },
    /**
    ...
    getActivity(activityId) {
      ...
      return bacsRequest
        .get(`activities/${activityId}?${paramsArr.join("&")}`)
        .then(res => {
          if (res.isDelete) {
